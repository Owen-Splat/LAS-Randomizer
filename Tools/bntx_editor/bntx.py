...
# The original file is part of the BNTX Editor made by AboodXD
# Edited by ProfessorLaw for Link's Awakening Randomizer usage.
# Original repository : https://github.com/aboood40091/BNTX-Editor
...
class File:
    ...
    def extract(self, index, BFRESPath, dontShowMsg=False):
        ...
        if (texture.format_ in globals.formats
                and texture.dim == 2
                and texture.arrayLength < 2
                and texture.tileMode in globals.tileModes):
    ...
    def replace(self, texture, tileMode, SRGB, sparseBinding, sparseResidency, importMips, f):
        ...
        if 0 in [width, dataSize] and data == []:
            print("Error - Unsupported DDS file!")
            return False

        if format_ not in globals.formats:
            print("Error - Unsupported DDS format!")
            return False
        ...
        texture.textureLayout = blockHeightLog2+2**32
    ...
    def save(self):
        # Relocation table sections init
        self.relocTblHeader.blockSize = 2
        self.relocTbl.blocks = [
            self.relocTbl.Block(self.header.endianness),
            self.relocTbl.Block(self.header.endianness)
        ]

        # Relocation table entries init
        self.relocTbl.entries = []

        # First section describe
        self.relocTbl.blocks[0].pos = 0
        self.relocTbl.blocks[0].relocEntryIdx = 0

        # TODO Determine what these entries are
        ...
        # TODO Determine where 0x198 leads
        pos = 0x198
        ...
        while count > 0:
            self.relocTbl.entries.append(self.relocTbl.Entry(self.header.endianness))
            self.relocTbl.entries[-1].pos = pos
            # Fixing this by adding one entry object per struct, and not one objet with number of structs
            # self.relocTbl.entries[-1].structs = [[pos + i * 8 for i in range(min(count, 0xFF))]]
            self.relocTbl.entries[-1].structs = []
            for i in range(min(count, 0xFF)):
                self.relocTbl.entries[-1].structs.append([pos + i * 8])
        ...
        for i in range(self.texContainer.count):
            ...
            # First user data entry (this one was missing in the original file)
            self.relocTbl.entries.append(self.relocTbl.Entry(self.header.endianness))
            self.relocTbl.entries[-1].pos = texture.pos + 0x68
            self.relocTbl.entries[-1].structs = [[texture.pos + 0x68]]
            self.relocTbl.entries[-1].paddingCount = 0

            # Second texture info entry
            self.relocTbl.entries.append(self.relocTbl.Entry(self.header.endianness))
            self.relocTbl.entries[-1].pos = texture.pos + 0x70
            self.relocTbl.entries[-1].structs = [[texture.pos + 0x70, texture.pos + 0x78]]
            self.relocTbl.entries[-1].paddingCount = 0

            # Second user data entry (this one was missing in the original file)
            self.relocTbl.entries.append(self.relocTbl.Entry(self.header.endianness))
            self.relocTbl.entries[-1].pos = texture.pos + 0x88
            self.relocTbl.entries[-1].structs = [[texture.pos + 0x88]]
            self.relocTbl.entries[-1].paddingCount = 0
        ...
        # We should have added every entry from the first section there.
